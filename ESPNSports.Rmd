...
TeamDataNew %>% group_by(Sport,Team,HowFar) %>% filter(City %in% c("Raleigh","Charlotte")) %>% summarise(count = n(), ESPN = ESPNRating) %>% select(Team,ESPN,HowFar) %>% kable() %>% kable_styling(c("striped","condensed"), full_width = F) %>% column_spec(column = 1:2, color = "black")
