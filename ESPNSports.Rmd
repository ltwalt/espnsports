...
TeamDataNew %>% group_by(City,Sport,Team,HowFar) %>% filter(City %in% "Raleigh") %>% summarise(count = n(), ESPN = ESPNRating) %>% select(City,Team,ESPN,HowFar) %>% kable() %>% kable_styling(c("striped","condensed"), full_width = F) %>% column_spec(column = 1:2, color = "black")
