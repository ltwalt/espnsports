...
## Overall
```{r}
TeamDataNew <- TeamData %>% group_by(Team) %>% mutate(HowFar = sum(X2014HF,X2015HF,X2016HF,X2017HF)/4)
...
TeamDataNew %>% group_by(City,Sport,Team,HowFar) %>% filter(City %in% c("Washington","Philadelphia","San Antonio","Tampa Bay")) %>% summarise(count = n(), ESPN = ESPNRating) %>% select(City,Team,ESPN,HowFar) %>% kable() %>% kable_styling(c("striped","condensed"), full_width = F) %>% column_spec(column = 1:2, color = "black")
...
wss <- (nrow(TeamDataKM2017)-1)*sum(apply(TeamDataKM2017,2,var))
for (i in 2:15) wss[i] <- sum(kmeans(TeamDataKM2017,
  	centers=i)$withinss)
...
ESPNAnalytics <- data.frame("Year" = c(2015,2016,2017),"High" = c(4.0,3.7,3.8),"Low" = c(2.2,2.9,2.9))
