...
UofArkPres <- read_pptx("201111 2020 - Is Analytics in Professional Sports Making a Difference - U of Ark.pptx")
...
TeamDataNew <- TeamData %>% 
  group_by(Team) %>% 
  mutate(HowFar = sum(X2014HF,X2015HF,X2016HF,X2017HF,X2018HF,X2019HF)/6)
...
ESPNRank <- TeamDataNew %>% 
  group_by(ESPNRating) %>% 
  summarise(Count = n(), HowFar = digits(sum(X2014HF,X2015HF,X2016HF,X2017HF,X2018HF)/(5*n()),1), MeanPop = digits(mean(PopulationMil),1), MeanSalary = digits(sum(X2014Salary,X2015Salary,X2016Salary,X2017Salary,X2018Salary,X2019Salary)/(6*n()),1)) %>% 
  flextable() %>%
#  colformat_num(j = 5, digits = 1) %>% 
  bold(part = "header") %>% 
  align(align = "center", part = "header" ) %>% 
...
UofArkPresNew <- UofArkPres %>%
  ...
  ph_with(SmallCity, location = ph_location(left = 3, top = 2))
...
ESPNAnalytics <- data.frame("Year" = c(2015,2016,2017,2018,2019),"High" = c(3.5,3.6,3.8,4.0,4.0),"Low" = c(3.1,3.0,2.9,2.9,2.9))
...
ESPNAnalyticsTime <- ESPNAnalytics %>% ggplot() +
  ...
  ggtitle("2015-2019 Group 1 vs Group 2")

UofArkPresNew <- UofArkPres %>%
  add_slide(master = "Black") %>% 
#  ph_with(value = "2014-2019 Group 1 vs Group 2", location = ph_location_type(type = "title")) %>% 
  ph_with(ESPNAnalyticsTime, location = ph_location_fullsize())

# UofArkPresNew <- UofArkPresNew %>% move_slide(index = 15, to = 9)

ESPNAnalyticsTime

print(UofArkPresNew, target = "Test.pptx")
